"""
Extração de times da API football-data.org.
...
def extract_teams(conn_id: str = CONN_ID_FOOTBALL_DATA_ORG) -> dict[str, Any]:
    """
    Extrai times de todas as competições configuradas (API football-data.org).
    ...
    for competition_id in COMPETITIONS:
        try:
            out[competition_id] = hook.get_competition_teams(competition_id)
        except Exception:
            continue
    return out
